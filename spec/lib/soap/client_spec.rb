  ...
  describe "web requests" do
    ...
    describe "upsert action" do
      ...
      context "with invalid inputs" do
        ...
        it "should not do anything when giving crap objects" do
          ret = @soap_client.upsert [1, "adkasdlsa", nil, true], "some_field"
          ret.empty?.should be_true
        end
        ...
        it "should not accept objects that do not have the rest_client set" do
          ...
          boombox.client = nil
          lambda {
            @soap_client.upsert boombox, "some_field"
          }.should raise_error(ArgumentError)
        end
      end
      ...
      context "with valid inputs" do
        ...
        it "should set ids when returning positive results from a single request" do
          @response_body = File.read(File.join(File.dirname(__FILE__), "../../fixtures/soap/upsert_positive_response_5_items.xml"))
          stub_request(:post, "https://foobar.com:80/services/Soap/c/21").with(:body => /<urn:upsert/).to_return(:body => @response_body)

          @boom_boxes[1].Id.should == "zwei"

          errors = @soap_client.upsert @boom_boxes, "some_field"
        ...
        it "should set ids when returning positive results from two requests" do
          @soap_client.record_limit = 3
          @response_bodies = File.read(File.join(File.dirname(__FILE__), "../../fixtures/soap/upsert_positive_response_5_items_in_two_requests.xml")).split("::::::")
          stub_request(:post, "https://foobar.com:80/services/Soap/c/21").with(:body => /eins/).to_return(:body => @response_bodies[0])
          stub_request(:post, "https://foobar.com:80/services/Soap/c/21").with(:body => /fuenf/).to_return(:body => @response_bodies[1])

          @boom_boxes[1].Id.should == "zwei"
          @soap_client.upsert @boom_boxes, "some_field"
        ...
        it "should not set ids when returning negative results and set the errors correctly" do
          @response_body = File.read(File.join(File.dirname(__FILE__), "../../fixtures/soap/upsert_positive_and_negative_response_5_items.xml"))
          stub_request(:post, "https://foobar.com:80/services/Soap/c/21").to_return(:body => @response_body)
        ...
        it "should set the external field id" do
          @response_body = File.read(File.join(File.dirname(__FILE__), "../../fixtures/soap/upsert_positive_response_5_items.xml"))
          stub_request(:post, "https://foobar.com:80/services/Soap/c/21").with(:body => /<urn:upsert/).to_return(:body => @response_body)
          ...
          errors.count.should == 0
          WebMock.should have_requested(:post, "https://foobar.com:80/services/Soap/c/21").with(:body => /<urn:externalIDFieldName>some_field<\/urn:externalIDFieldName>/) 
        end     
      end
    end
  end
